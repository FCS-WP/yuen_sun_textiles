...
//							fabric_document_animate_to('#content_skip_link_anchor');
							...
							setTimeout( function() {
								panel
									.find( '.fabric_tab_holder,.fabric_tab_content_remove' ).remove();
								$window.trigger( 'scroll' );
							}, 300 );
						}
					);
		...
		if ( FABRIC_STORAGE['resize_tag_iframe'] ) {
			cont.find( '.video_frame iframe,iframe' ).each( function() {
				...
				if ( $self.hasClass( 'trx_addons_resize' )
					...
					|| $self.addClass( 'fabric_resize' ).parents( 'div:hidden,section:hidden,article:hidden' ).length > 0
				) {
					return;
				}
				...
				if ( ! w_attr || ! h_attr || w_attr <= fabric_apply_filters( 'fabric_filter_noresize_iframe_width', 325 ) ) {
					return;
				}
