...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			fabric_create_observer( 'fabric-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				fabric_gutenberg_editor_init();
			} );
		} else {
		 	fabric_gutenberg_editor_init();
		}
	...
	function fabric_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'fabric_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'fabric_inited' ) ) {
	...
	// Init on page load
	function fabric_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = fabric_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + FABRIC_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + FABRIC_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in FABRIC_STORAGE['override_classes'] ) {
				$obj.addClass( FABRIC_STORAGE['override_classes'][i].replace( '%s', FABRIC_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( fabric_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && FABRIC_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'fabric_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			fabric_remove_observer( 'fabric-check-editor-styles-wrapper' );
			fabric_create_observer( 'fabric-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'fabric_inited' );
	}
} );
