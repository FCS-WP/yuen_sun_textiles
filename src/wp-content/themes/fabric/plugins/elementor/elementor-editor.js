...
jQuery( document ).ready(
	function() {
		...
		fabric_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="fabric_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.fabric_post_editor' );
						if ( link.length > 0 ) {
							fabric_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.fabric_post_editor', function(e) {
						fabric_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('fabric_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function fabric_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
