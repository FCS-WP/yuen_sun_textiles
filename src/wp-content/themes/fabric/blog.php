...
if ( function_exists( 'fabric_elementor_is_preview' ) && fabric_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			fabric_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	fabric_new_main_query(
		array(
			'post_type'      => fabric_get_theme_option( 'post_type' ),
			'category'       => fabric_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'fabric_filter_get_template_part', fabric_blog_archive_get_template() ) );
}
