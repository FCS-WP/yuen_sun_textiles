<div class="front_page_section front_page_section_blog<?php
	$fabric_scheme = fabric_get_theme_option( 'front_page_blog_scheme' );
	if ( ! empty( $fabric_scheme ) && ! fabric_is_inherit( $fabric_scheme ) ) {
		echo ' scheme_' . esc_attr( $fabric_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( fabric_get_theme_option( 'front_page_blog_paddings' ) );
	if ( fabric_get_theme_option( 'front_page_blog_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$fabric_css      = '';
		$fabric_bg_image = fabric_get_theme_option( 'front_page_blog_bg_image' );
...
<?php
	// Add anchor
	$fabric_anchor_icon = fabric_get_theme_option( 'front_page_blog_anchor_icon' );
	$fabric_anchor_text = fabric_get_theme_option( 'front_page_blog_anchor_text' );
...
?>
	...
	<?php
	if ( fabric_get_theme_option( 'front_page_blog_fullheight' ) ) {
	...
	?>
			...
			<?php
			$fabric_css      = '';
			$fabric_bg_mask  = fabric_get_theme_option( 'front_page_blog_bg_mask' );
			$fabric_bg_color_type = fabric_get_theme_option( 'front_page_blog_bg_color_type' );
			if ( 'custom' == $fabric_bg_color_type ) {
				$fabric_bg_color = fabric_get_theme_option( 'front_page_blog_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_blog_content_wrap content_wrap">
			<?php
			// Caption
			$fabric_caption = fabric_get_theme_option( 'front_page_blog_caption' );
			if ( ! empty( $fabric_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_blog_caption front_page_block_<?php echo ! empty( $fabric_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $fabric_caption, 'fabric_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$fabric_description = fabric_get_theme_option( 'front_page_blog_description' );
			if ( ! empty( $fabric_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_blog_description front_page_block_<?php echo ! empty( $fabric_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $fabric_description ), 'fabric_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_blog_output">
				...
				if ( is_active_sidebar( 'front_page_blog_widgets' ) ) {
					...
					if ( ! fabric_exists_trx_addons() ) {
						...
						fabric_customizer_need_widgets_message( 'front_page_blog_caption', 'ThemeREX Addons - Blogger' );
					}
				}
				?>
			</div>
