...
if ( ! function_exists( 'fabric_storage_get' ) ) {
	/**
	 ...
	 */
	function fabric_storage_get( $var_name, $default = '' ) {
		global $FABRIC_STORAGE;
		return isset( $FABRIC_STORAGE[ $var_name ] ) ? $FABRIC_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'fabric_storage_set' ) ) {
	/**
	 ...
	 */
	function fabric_storage_set( $var_name, $value ) {
		global $FABRIC_STORAGE;
		$FABRIC_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'fabric_storage_empty' ) ) {
	/**
	 ...
	 */
	function fabric_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $FABRIC_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $FABRIC_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $FABRIC_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $FABRIC_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'fabric_storage_isset' ) ) {
	/**
	 ...
	 */
	function fabric_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $FABRIC_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $FABRIC_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $FABRIC_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $FABRIC_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'fabric_storage_unset' ) ) {
	/**
	 ...
	 */
	function fabric_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $FABRIC_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $FABRIC_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $FABRIC_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $FABRIC_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'fabric_storage_inc' ) ) {
	/**
	 ...
	 */
	function fabric_storage_inc( $var_name, $value = 1 ) {
		global $FABRIC_STORAGE;
		if ( empty( $FABRIC_STORAGE[ $var_name ] ) ) {
			$FABRIC_STORAGE[ $var_name ] = 0;
		}
		$FABRIC_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'fabric_storage_concat' ) ) {
	/**
	 ...
	 */
	function fabric_storage_concat( $var_name, $value ) {
		global $FABRIC_STORAGE;
		if ( empty( $FABRIC_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'fabric_storage_get_array' ) ) {
	/**
	 ...
	 */
	function fabric_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $FABRIC_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $FABRIC_STORAGE[ $var_name ][ $key ] ) ? $FABRIC_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $FABRIC_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $FABRIC_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'fabric_storage_set_array' ) ) {
	/**
	 ...
	 */
	function fabric_storage_set_array( $var_name, $key, $value ) {
		global $FABRIC_STORAGE;
		if ( ! isset( $FABRIC_STORAGE[ $var_name ] ) ) {
			$FABRIC_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$FABRIC_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'fabric_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function fabric_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $FABRIC_STORAGE;
		if ( ! isset( $FABRIC_STORAGE[ $var_name ] ) ) {
			$FABRIC_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $FABRIC_STORAGE[ $var_name ][ $key ] ) ) {
			$FABRIC_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$FABRIC_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$FABRIC_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'fabric_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function fabric_storage_merge_array( $var_name, $key, $value ) {
		global $FABRIC_STORAGE;
		if ( ! isset( $FABRIC_STORAGE[ $var_name ] ) ) {
			$FABRIC_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$FABRIC_STORAGE[ $var_name ] = array_merge( $FABRIC_STORAGE[ $var_name ], $value );
		} else {
			$FABRIC_STORAGE[ $var_name ][ $key ] = array_merge( $FABRIC_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'fabric_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function fabric_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $FABRIC_STORAGE;
		if ( ! isset( $FABRIC_STORAGE[ $var_name ] ) ) {
			$FABRIC_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			fabric_array_insert_after( $FABRIC_STORAGE[ $var_name ], $after, $key );
		} else {
			fabric_array_insert_after( $FABRIC_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'fabric_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function fabric_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $FABRIC_STORAGE;
		if ( ! isset( $FABRIC_STORAGE[ $var_name ] ) ) {
			$FABRIC_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			fabric_array_insert_before( $FABRIC_STORAGE[ $var_name ], $before, $key );
		} else {
			fabric_array_insert_before( $FABRIC_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'fabric_storage_push_array' ) ) {
	/**
	 ...
	 */
	function fabric_storage_push_array( $var_name, $key, $value ) {
		global $FABRIC_STORAGE;
		if ( ! isset( $FABRIC_STORAGE[ $var_name ] ) ) {
			$FABRIC_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $FABRIC_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $FABRIC_STORAGE[ $var_name ][ $key ] ) ) {
				$FABRIC_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $FABRIC_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'fabric_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function fabric_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $FABRIC_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $FABRIC_STORAGE[ $var_name ] ) && is_array( $FABRIC_STORAGE[ $var_name ] ) && count( $FABRIC_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $FABRIC_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $FABRIC_STORAGE[ $var_name ][ $key ] ) && is_array( $FABRIC_STORAGE[ $var_name ][ $key ] ) && count( $FABRIC_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $FABRIC_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'fabric_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function fabric_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $FABRIC_STORAGE;
		if ( ! isset( $FABRIC_STORAGE[ $var_name ] ) ) {
			$FABRIC_STORAGE[ $var_name ] = array();
		}
		if ( empty( $FABRIC_STORAGE[ $var_name ][ $key ] ) ) {
			$FABRIC_STORAGE[ $var_name ][ $key ] = 0;
		}
		$FABRIC_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'fabric_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function fabric_storage_concat_array( $var_name, $key, $value ) {
		global $FABRIC_STORAGE;
		if ( ! isset( $FABRIC_STORAGE[ $var_name ] ) ) {
			$FABRIC_STORAGE[ $var_name ] = array();
		}
		if ( empty( $FABRIC_STORAGE[ $var_name ][ $key ] ) ) {
			$FABRIC_STORAGE[ $var_name ][ $key ] = '';
		}
		$FABRIC_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'fabric_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function fabric_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $FABRIC_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $FABRIC_STORAGE[ $var_name ] ) ? $FABRIC_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $FABRIC_STORAGE[ $var_name ] ) ? $FABRIC_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'fabric_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function fabric_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $FABRIC_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $FABRIC_STORAGE[ $var_name ]->$prop ) ? $FABRIC_STORAGE[ $var_name ]->$prop : $default;
	}
}
